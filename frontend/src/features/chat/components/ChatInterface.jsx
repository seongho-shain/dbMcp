...
function ChatInterface({ sessionId, sessionInfo }) {
  const { user } = useAuth()
  ...
  const loadChatHistory = async () => {
    try {
      const currentSessionId = sessionId || user.session_id
      const response = await fetch(`${API_BASE_URL}/chat/history/${user.id}/${currentSessionId}`)
      if (response.ok) {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      formData.append('session_id', sessionId || user.session_id)
  ...
  // 컴포넌트 마운트 시 채팅 기록 로드
  useEffect(() => {
    if (user && user.id && (sessionId || user.session_id)) {
      loadChatHistory()
    }
  }, [user, sessionId])
